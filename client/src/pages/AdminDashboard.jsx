  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <form onSubmit={submitProduct} className="panel space-y-3 p-5">
          ...
          <button className="btn-primary">{editingId ? <Save className="h-4 w-4" /> : <Plus className="h-4 w-4" />} Save Product</button>
        </form>
        <div className="panel p-5">
          <h2 className="mb-4 text-xl font-bold">Products</h2>
          <div className="max-h-[520px] space-y-3 overflow-auto pr-2">
            {products.map((product) => (
              <div className="flex items-center justify-between rounded-md bg-slate-50 p-3" key={product._id}>
                <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="btn-secondary px-3" onClick={() => remove(product._id)}><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="panel mt-6 p-5">
        ...
        <div className="space-y-3">
          {orders.map((order) => (
            <div className="grid gap-3 rounded-md bg-slate-50 p-3 md:grid-cols-[1fr_auto_auto]" key={order._id}>
              <div>
